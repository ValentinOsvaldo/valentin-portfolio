...
export const TimelineItem: React.FC<TimelineItemProp> = ({
  ...
  return (
    <li className="mb-10 ms-4">
      ...
      <time className="mb-1 text-sm font-normal leading-none dark:text-neutral-400">
        {starDate} {endDate && `- ${endDate}`}
      </time>
      <h3 className="text-lg font-semibold">{`${jobTitle} in ${company}`}</h3>
      {technologies && (
      ...
      {description && <p className="dark:text-neutral-400 text-sm">{description}</p>}
      {featured && (
